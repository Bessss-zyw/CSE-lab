...
yfs_client::isfile(inum inum)
{
    ...
    if (a.type == extent_protocol::T_FILE) {
        if (DEBUG) printf("yc::isfile: %lld is a file\n", inum);
        return true;
    } 

    if (DEBUG) printf("yc::isfile: %lld is not a file\n", inum);
    return false;
}
...
yfs_client::isdir(inum inum)
{
    ...
    if (a.type == extent_protocol::T_DIR) {
        if (DEBUG) printf("yc::isdir: %lld is a dir\n", inum);
        return true;
    } 
    if (DEBUG) printf("yc::isdir: %lld is not a dir\n", inum);
    return false;
}
...
yfs_client::issymlink(inum inum)
{
    ...
    if (a.type == extent_protocol::T_SYMLINK) {
        if (DEBUG) printf("yc::issymlink: %lld is a link\n", inum);
        return true;
    } 
    if (DEBUG) printf("yc::issymlink: %lld is not a link\n", inum);
    return false;
}
...
yfs_client::getfile(inum inum, fileinfo &fin)
{
    ...
    if (DEBUG) printf("yc::getfile %016llx\n", inum);
    extent_protocol::attr a;
    lc->acquire(inum);
    if (ec->getattr(inum, a) != extent_protocol::OK || a.atime == 0) {
        r = IOERR;
    ...
    fin.ctime = a.ctime;
    fin.size = a.size;
    if (DEBUG) printf("yc::getfile %016llx -> size %llu\n", inum, fin.size);
...
yfs_client::getdir(inum inum, dirinfo &din)
{
    ...
    if (DEBUG) printf("yc::getdir %016llx\n", inum);
    extent_protocol::attr a;
    lc->acquire(inum);
    if (ec->getattr(inum, a) != extent_protocol::OK || a.atime == 0) {
        r = IOERR;
    ...
    din.mtime = a.mtime;
    din.ctime = a.ctime;
    if (DEBUG) printf("yc::getdir %016llx done\n", inum);
...
yfs_client::setattr(inum ino, size_t size)
{
     ...
     */
    if (DEBUG) printf("yc::setattr %016llx to size %lu\n", ino, size);
...
yfs_client::create(inum parent, const char *name, mode_t mode, inum &ino_out)
{
     ...
     */
    if (DEBUG) printf("yc::create parent %llu\n", parent);
...
yfs_client::mkdir(inum parent, const char *name, mode_t mode, inum &ino_out)
{
     ...
     * after create file or dir, you must remember to modify the parent infomation.
     */
    if (DEBUG) printf("yc::mkdir parent %llu\n", parent);
...
{
    if (DEBUG) printf("yc::lookup parent %llu, name = %s\n", parent, name);
...
{
    if (DEBUG) printf("yc::readdir dir %llu\n", dir);
...
yfs_client::read(inum ino, size_t size, off_t off, std::string &data)
{
     ...
     */
    if (DEBUG) printf("yc::read %016llx, size %lu, off = %ld\n", ino, size, off);
    ...
    if (DEBUG) printf("yc::read file_size %lu\n", size_origin);    
        ...
        size_t &bytes_written)
{
     ...
     */
    if (DEBUG) printf("yc::write %016llx, size %lu, off = %ld\n", ino, size, off);
    lc->acquire(ino);
    if (ec->get(ino, content) != extent_protocol::OK) {
        r = IOERR;
        goto release;
    }
    if (DEBUG) printf("yc::write get %016llx done\n", ino);

    size_origin = content.size();
    if ((uint32_t)off > size_origin){
        if (DEBUG) printf("yc::write %016llx, off = %ld > size_origin = %lu\n", ino, off, size_origin);
    ...
    if (DEBUG) printf("yc::write %016llx, replace done, write_after size = %lu\n", ino, content.size());
...
yfs_client::unlink(inum parent,const char *name)
{
     ...
     */
    if (DEBUG) printf("yc::unlink %016llx, name = %s\n", parent, name);
...
yfs_client::readlink(inum ino, std::string &buf){
    int r = OK;
    if (DEBUG) printf("yc::readlink ino %llu\n", ino);
...
yfs_client::symlink(inum parent, const char *name, const char *link, inum &ino_out){
     ...
     * after create file or dir, you must remember to modify the parent infomation.
     */
    if (DEBUG) printf("yc::symlink parent %llu, name = %s, link = %s\n", parent, name, link);
...
yfs_client::__lookup(inum parent, const char *name, bool &found, inum &ino_out)
{
     ...
     */
    if (DEBUG) printf("yc::__lookup parent %llu, name = %s\n", parent, name);
...
yfs_client::__readdir(inum dir, std::list<dirent> &list)
{
     ...
     */
    if (DEBUG) printf("yc::__readdir dir %llu\n", dir);
